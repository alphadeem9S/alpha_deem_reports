...
class Project(Document):
	...
	def update_percent_complete(self):
		self.days = date_diff(self.expected_end_date, self.expected_start_date)
	...
	def update_costing(self):
		...
		from_expense_claim = frappe.db.sql(
			...
			from `tabExpense Claim` where cost_center = %s
			and docstatus = 1""",
			self.cost_center,
			as_dict=1,
		)[0]
		...
		self.update_delivery_amount()
		self.update_je_expenses_amount()
		self.calculate_gross_margin()

	def calculate_gross_margin(self):
		self.tax_amount = flt(self.project_amount) * (flt(self.tax) * 0.01)
		self.grand_total = flt(self.project_amount) + flt(self.tax_amount)
		...
		expense_amount = (
			...
			+ flt(self.total_delivery_cost)
			+ flt(self.total_expenses)
		)

		self.actual_cost = expense_amount
		self.variance = flt(self.estimated_costing) - flt(self.actual_cost)

		self.percent_complete = (flt(self.total_billed_amount) / flt(self.grand_total)) * 100 if self.grand_total else 0
		...
		if self.status == "Open":
			...
			projects = frappe.db.sql("""
        			...
        			FROM `tabProject`
        			WHERE company = %(company)s and status = %(status)s
    			""", values=values, as_dict=1)
			total = sum(p.grand_total for p in projects)

			for p in projects:
				doc = frappe.get_doc("Project", p.name)
				percentage = (flt(doc.grand_total) / flt(total)) * 100 if flt(total) > 0 else 100
				doc.db_set("project_percentage", percentage, update_modified=False)
		else:
			self.project_percentage = 0

		for d in self.get("accounts"):
			if d.budget_account:
				values = {
        				"parent_account": d.budget_account,
				...
				total_amount = frappe.db.sql("""
					...
					AND account in (select name from `tabAccount` where parent_account=%(parent_account)s)
        			""", values=values, as_dict=1)[0]

				d.account_budget = flt(total_amount.total_debit) - flt(total_amount.total_credit)
			
			d.account_variance = flt(d.account_estimated_cost) - frappe.utils.flt(d.account_budget)
		
		self.total_account_estimated_cost = sum(flt(d.account_estimated_cost) for d in self.get("accounts"))
		self.total_account_budget = sum(flt(d.account_budget) for d in self.get("accounts"))
		self.total_account_variance = sum(flt(d.account_variance) for d in self.get("accounts"))
	...
	def update_purchase_costing(self):
		total_purchase_cost = frappe.db.sql(
			...
			from `tabPurchase Invoice Item` where cost_center = %s and docstatus=1""",
			self.cost_center,
		)
	...
	def update_sales_amount(self):
		total_sales_amount = frappe.db.sql(
			...
			from `tabSales Order` where cost_center = %s and docstatus=1""",
			self.cost_center,
		)
	...
	def update_billed_amount(self):
		total_billed_amount = frappe.db.sql(
			"""select sum(base_net_total)
			from `tabSales Invoice` where cost_center = %s and docstatus=1""",
			self.cost_center,
		)

		self.total_billed_amount = total_billed_amount and total_billed_amount[0][0] or 0

	def update_delivery_amount(self):
		total_delivery_amount = frappe.db.sql(
			"""select sum(base_net_total)
			from `tabDelivery Note` where cost_center = %s and docstatus=1""",
			self.cost_center,
		)

		self.total_delivery_cost = total_delivery_amount and total_delivery_amount[0][0] or 0

	def update_je_expenses_amount(self):
		total_expenses_amount = frappe.db.sql(
			"""select sum(debit_in_account_currency)
			from `tabJournal Entry Account` where cost_center = %s and docstatus=1""",
			self.cost_center,
		)

		self.total_expenses = total_expenses_amount and total_expenses_amount[0][0] or 0
